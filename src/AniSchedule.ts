...
async function handleSchedules(time: number, page: number) {
  ...
  response.data.Page.airingSchedules.forEach((e: any) => {
    ...
    console.log(`Scheduling announcement for ${e.media.title.english} on ${date.toLocaleDateString('en-US', { timeZone: 'Asia/Manila' })} at ${date.toLocaleTimeString('en-US', { timeZone: 'Asia/Manila' })}`);
...
function makeAnnouncement(entry: any, date: Date, upNext = false) {
  ...
  storage.servers.forEach(server => {
    server.channels.forEach(c => {
      ...
      if (channel) {
        console.log(`Announcing episode ${entry.media.title.english} to ${channel.guild.name}@${channel.id}`);
